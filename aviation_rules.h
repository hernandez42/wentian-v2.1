...
/* ═══════════════════════════════════════════════════════════ */
/* B737-700/800 起飞性能数据 (CCAR-121 §121.189 高原性能基准) */
/* ============================================================
 * 数据来源: B737 FCOM / QRH 高原起飞表 (典型值, 襟翼5, 引气正常)
 * 结构: {密度高度DA(ft), 温度(°C), 起飞重量(kg), 起飞距离(m)}
 * 插值: wt_b737_takeoff_dist() 按DA-温度-重量三维线性插值
 * ============================================================ */

typedef struct {
    int da_ft;          /* 密度高度(ft) */
    int temp_c;         /* 外界温度(°C) */
    int weight_kg;      /* 起飞重量(kg) */
    int takeoff_dist_m; /* 起飞距离(m) */
} b737_perf_t;

/* 6个典型DA × 2个温度 × 2个重量 = 24个性能点 */
static const b737_perf_t B737_PERF_TABLE[24] = {
    /* DA=0ft (海平面) */
    {    0, 15, 65000, 1800},
    {    0, 15, 70000, 2100},
    {    0, 30, 65000, 2100},
...
/* 查表用的DA断点(ft) — 必须与B737_PERF_TABLE的DA行顺序一致 */
#define B737_DA_STEPS   {0, 2000, 4000, 6000, 8000, 10000}
#define B737_TEMP_STEPS {15, 30}
#define B737_WEIGHT_STEPS {65000, 70000}
